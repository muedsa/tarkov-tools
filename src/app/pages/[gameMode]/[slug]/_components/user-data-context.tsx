...
function userDataReducer(state: UserData, action: UserDataAction) {
  if (action.type === "CHANGE_HIDEOUT_STATION_LEVEL") {
    ...
    storeLocalUserData(state, action.gameMode);
    return {
      ...state,
...
export function UserDataProvider({
  ...
  useEffect(() => {
    try {
      dispatch({
        type: "LOAD_STATE",
        state: getLocalUserData(gameMode),
        gameMode,
      });
    } catch (error) {
  ...
  const changeHideoutStationLevel = (id: string, count: number) => {
    dispatch({
      type: "CHANGE_HIDEOUT_STATION_LEVEL",
      id: id,
      count: count,
      gameMode: gameMode,
    });
  ...
  const addCompletedTask = (id: string) => {
    dispatch({ type: "ADD_COMPLETEED_TASK", id: id, gameMode: gameMode });
  };

  const removeCompletedTask = (id: string) => {
    dispatch({ type: "REMOVE_COMPLETEED_TASK", id: id, gameMode: gameMode });
  };

  const changeCollectedItem = (id: string, count: number) => {
    dispatch({
      type: "CHNAGE_COLLECTED_ITEM",
      id: id,
      count: count,
      gameMode: gameMode,
    });
